...
export class PresenceValidator {
    ...
    public static hasAllRequiredFields<T, const F extends readonly (keyof T)[]>(
        ...
        for (const field of requiredFields) {
            if (object[field] === undefined || object[field] === null) {
                throw new ReferenceError(`Required field ${String(field)} is null or undefined`);
            }
        }
    ...
    public static validateHttpResponseBody<T extends { body?: B }, B>(response: T) {
        return this.hasAllRequiredFields(response, ['body']);
    }

    public static validateOffsetPaginatedResource<
        C, T extends { content?: C[], pagination?: JavaOffsetPaginationDTO }
    >(resource: T): {
        content: C[], pagination: WithPresentFields<
            JavaOffsetPaginationDTO,
            'number' | 'total_pages' | 'number_of_elements' | 'size' | 'total_elements'
        >
    } {
        return this.hasAllRequiredFields(
            {
                ...resource, pagination: this.hasAllRequiredFields(
                    resource.pagination,
                    ['number', 'total_pages', 'number_of_elements', 'size', 'total_elements']
                )
            },
