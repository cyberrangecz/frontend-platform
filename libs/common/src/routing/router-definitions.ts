...
type RouteNames = { [key: string]: ValidPath | RouteNames };

/**
 * Function which correctly narrows down the tye
 * @param routes
 */
function defineNamedRoutes<T extends RouteNames>(routes: T): T {
    return routes;
}

export const NAMED_ROUTES = defineNamedRoutes({
    HOME: 'home',
    LOGIN: 'login',
    LOGOUT: 'logout',
...
function handleExclVar(pathSegment: string): [excl: boolean, variable: boolean] {
    const exclVar = pathSegment.startsWith("EXCL_VAR_") || pathSegment.startsWith("VAR_EXCL_");
...
export const VALID_ROUTES = generateValidRoutes(DEFINED_ROUTES) as { [K in ValidPath]: true };
...
function createNavigationBuilder<T extends Record<string, any>, P extends string = ''>(
    routes: T,
    basePath: P = '' as P
): NavigationBuilder<T, P> {
    const builder: any = {};
