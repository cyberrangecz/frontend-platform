...
import {PortalDynamicEnvironment} from '../../../../apps/cyberrangecz-platform/src/app/portal-dynamic-environment';
import {PortalConfig} from "../index";
...
export class ErrorHandlerService implements ErrorHandler {
    ...
    emit(err: HttpErrorResponse, operation: string, action?: string): Observable<boolean> {
        ...
        if (
            err.url?.startsWith(config.basePaths.linearTraining) ||
            err.url?.startsWith(config.basePaths.adaptiveTraining)
        ) {
            this.setJavaApiErrorNotification(err, notification);
            notification.source = 'Training Agenda';
        } else if (err.url?.startsWith(config.basePaths.userAndGroup)) {
            this.setJavaApiErrorNotification(err, notification);
            notification.source = 'User & Group Agenda';
        } else if (err.url?.startsWith(config.basePaths.sandbox)) {
